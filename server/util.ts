...
export function getCfenv() {
	...
	const serviceName =
		process.env.STT_SERVICE_NAME || 'code-pattern-custom-language-model';
...
export class WatsonSTT {
	...
	private constructor(
		...
		baseModel: string,
		speechModels: STTDef.SpeechModels
	) {
		this.speech = speech;
	...
	async trainModel(): Promise<[STTError]> {
		return new Promise<[STTError]>((resolve, reject) => {
			this.speech.trainLanguageModel(
				{ customization_id: this.langModelId },
				(error: STTError) => {
					if (error) {
						resolve([error]);
	...
	async getLanguageModel(): Promise<[STTError, STTDef.LanguageModel?]> {
		return new Promise<[STTError, STTDef.LanguageModel?]>((resolve, reject) => {
			this.speech.getLanguageModel(
				{ customization_id: this.langModelId },
	...
	isBaseModel(model: string): boolean {
		let found = false;
		if (
			this.speechModels.models.find((element): boolean => {
	...
	async getAcousticModel(): Promise<[STTError, STTDef.AcousticModel?]> {
		return new Promise<[STTError, STTDef.AcousticModel?]>((resolve, reject) => {
	...
	async trainAcousticModel(): Promise<[STTError]> {
		...
		return new Promise<[STTError]>((resolve, reject) => {
			this.speech.trainAcousticModel(
...
function getBaseModels(
	...
	return new Promise<[STTError, STTDef.SpeechModels?]>((resolve, reject) => {
		speech.listModels(null, (error: STTError, results: STTDef.SpeechModels) => {
			if (error) {
				resolve([error]);
			} else {
				resolve([undefined, results]);
